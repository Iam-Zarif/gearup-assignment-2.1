...
export default function GearDetails({ id }: { id: string }) {
  ...
  useEffect(() => {
    let isMounted = true;
    void Promise.all([getGearById(id), getGearReviews(id)]).then(
      ([nextGear, nextReviews]) => {
        if (isMounted) {
          setGear(nextGear);
  ...
  async function submitRental(event: FormEvent<HTMLFormElement>) {
    ...
    if (user.role !== "CUSTOMER") {
      setRentalError("Only customer accounts can create rental orders.");
      return;
    }
    const values = new FormData(event.currentTarget);
    const quantity = Number(values.get("quantity"));
    if (!startDate || !endDate || !quantity) {
      setRentalError("Select rental dates and quantity.");
      return;
    }
    if (endDate < startDate) {
      setRentalError("End date cannot be before start date.");
      return;
    }
    ...
    try {
      ...
      setRentalError(
        getApiErrorMessage(requestError, "Unable to create rental order"),
      );
    } finally {
    ...
    <main className="mx-auto w-full max-w-7xl px-4 py-10">
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="space-y-5 col-span-2">
          ...
          <div>
            <p className="text-sm text-primary">{gear.category.name}</p>
            <h1 className="mt-1 text-4xl font-bold">{gear.name}</h1>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {Object.entries(gear.specifications ?? {}).map(([label, value]) => (
          ...
          <div className="rounded-xl border p-4">
            ...
            <p className="mt-2">{gear.provider.name}</p>
        ...
        <aside className="h-fit sticky top-20 rounded-2xl border bg-card p-6 shadow-sm">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                {gear.availableQuantity} of {gear.stockQuantity} available
              </p>
            </div>
          ...
          <form className="mt-6 space-y-4" onSubmit={submitRental}>
            ...
            <DatePicker
              label="End date"
              minDate={new Date(`${startDate || today}T00:00:00`)}
              name="endDate"
              onChange={setEndDate}
              value={endDate}
            />
      ...
      <section className="mt-12">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          {reviews.map((review) => (
            <article className="rounded-xl border p-4" key={review.id}>
