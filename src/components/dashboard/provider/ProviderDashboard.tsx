...
function ProviderDashboardContent() {
  ...
  return <section className="space-y-6">
    <div><h1 className="text-3xl font-bold tracking-tight">Provider dashboard</h1><p className="mt-1 text-muted-foreground">Live inventory and rental activity.</p></div>
    ...
    <section className="rounded-xl border"><div className="border-b p-4 font-semibold">Recent orders</div><div className="divide-y">{orders.slice(0, 5).map((order) => <div className="flex items-center justify-between gap-4 p-4 text-sm" key={order.id}><div><p className="font-medium">{order.customer.name}</p><p className="text-muted-foreground">{order.items.map((item) => item.gearItem.name).join(", ")}</p></div><span>{order.status}</span></div>)}{!isLoading && !orders.length ? <p className="p-8 text-center text-sm text-muted-foreground">No orders yet.</p> : null}</div></section>
  </section>;
}
